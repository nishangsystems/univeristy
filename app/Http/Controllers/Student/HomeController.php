...
class HomeController extends Controller
{
    ...
    public function ca_result(Request $request)
    {
        ...
        $results = array_map(function($subject_id)use($user, $year, $semester){
            return [
                ...
                'ca_mark'=>$user->result()->where(['results.batch_id'=> $year->id, 'results.subject_id'=> $subject_id, 'results.semester_id'=> $semester->id])->first()->ca_score ?? '',
            ];
        // }, $res);
        }, $registered_courses);
        // dd($data['results']);
    ...
    public function ca_result_download(Request $request)
    {
        ...
        $results = array_map(function($subject_id)use($data, $year, $semester){
            return [
                ...
                'status'=>Subjects::find($subject_id)->status ?? '',
                'coef'=>Subjects::find($subject_id)->coef ?? '',
                'ca_mark'=>$data['user']->result()->where('results.batch_id', '=', $year->id)->where('results.subject_id', '=', $subject_id)->where('results.semester_id', '=', $semester->id)->first()->ca_score ?? '',
            ];
        // }, $res);
        }, $registered_courses);
        $data['results'] = collect($results)->filter(function($el){return $el != null;});
    ...
    public function exam_result(Request $request)
    {
        ...
        try {
            ...
            $fee = [
                ...
                'total_paid'=>$data['user']->total_paid($year->id),
                'total' => $data['user']->total($year->id),
                'balance' => $data['user']->bal(null, $year->id),
                'total_balance' => $data['user']->total_balance(null, $year->id),
            ...
            if($data['access']){
                ...
                $sum_earned_cv = collect($results)->filter(function($el){return ($el != null) && (($el['ca_mark'] ?? 0)+($el['exam_mark'] ?? 0) >= 50);})->sum('coef');
                $gpa_cv = $data['results']->whereNotIn('id', $non_gpa_courses)->where('coef', '>', 0)->sum('coef');
                $gpa_cv_earned = $data['results']->whereNotIn('id', $non_gpa_courses)->filter(function($el){return ($el != null) && (($el['ca_mark'] ?? 0)+($el['exam_mark'] ?? 0) >= 50);})->where('coef', '>', 0)->sum('coef');
    ...
    public function exam_result_download(Request $request)
    {
        ...
        try {
            ...
            $data['year'] = $year;
            $data['semester'] = $semester;
            $data['ca_total'] = $data['user']->_class($year)->program()->first()->ca_total;
            $data['exam_total'] = $data['user']->_class($year)->program()->first()->exam_total;
            ...
            return $pdf->download($data['user']->matric.'_'.$semester->name.'_EXAM_RESULTS.pdf');
    ...
    public function course_registration()
    {
        try {
            ...
            // throw $th;
            return back()->with('error', 'F:__'.$th->getFile().'__Line:____'.$th->getLine().'----Message:----'.$th->getMessage());
        }
    }
    ...
    public function form_b()
    {
        ...
        $data['title'] = "Registered Courses ".(Helpers::instance()->getSemester($class->id)->name ?? "??? SEMESTER")." ".(Batch::find(Helpers::instance()->getCurrentAccademicYear())->name ?? "??? YEAR");
    ...
    public function resit_index()
    {
        ...
        $data['resits'] = Resit::join('student_courses', 'student_courses.resit_id', '=', 'resits.id')->where('student_courses.student_id', $user->id)->distinct()->get(['resits.*']);
        ...
        try {
            ...
            switch ($request->status) {
                case 'SUCCESSFUL':
                    ...
                    if (TranzakTransaction::where(['request_id'=>$transaction['request_id']])->orWhere(['transaction_id'=>$transaction['transaction_id']])->orWhere(['transaction_ref'=>$transaction['transaction_ref']])->count() > 0) {
                        # code...
                        // transaction already exist
                        return redirect(route('student.home'))->with('message', 'Transaction already exist in database');
                    }
