...
class CourseNotification extends Model
{
    ...
    {
        # code...
        return (Campus::find($this->campus_id)->name ?? '').' '.($this->classSubject() != null ? $this->classSubject()->class()->first()->program->name : ''). ' - '.($this->classSubject() != null ? $this->classSubject()->class()->first()->level->level : '').' '.($this->classSubject() != null ? $this->classSubject()->subject->name : '') .' Students.';
    }
}
