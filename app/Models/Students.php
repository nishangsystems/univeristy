...
class Students extends Authenticatable
{
    ...
    protected $fillable = [
        ...
        'region', 'division', 'nationality', 'deleted_at', 'program_status', 'parent_phone_code'
    ];
    ...
    public function total_debts($year)
    {
        ...
        foreach (Batch::where('id', '<', $year+1)->where('id', '>', $this->admission_batch_id-1)->get() as $key => $batch) {
            ...
            $dd[] = [$class->campus_programs($this->campus_id)->first()->payment_items()->where('year_id', $batch->id)->get()];
