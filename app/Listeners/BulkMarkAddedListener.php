...
class BulkMarkAddedListener
{
    ...
    public function handle($event)
    {
        ...
        $description = ($background == null ? '' : 'BACKGROUND: '.$background->background_name).":: {$event->action}: {$event->additional_mark} ADDED FOR [{$course->code}] {$course->name}, RANGE: {$event->range}, ".($background != null ? null : $class->name()??null)." :: {$semester->name} {$year->name}, BY: {$event->actor->name}";
