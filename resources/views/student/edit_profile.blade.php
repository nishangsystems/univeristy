                    ...
                    <div class="col-md-5">
                        <select class="form-control border rounded" id="country_picker" required name="parent_phone_code" onchange="set_field_length(this)">
                            ...
                            @foreach (collect(config('country-phone-data'))->sortBy('label') as $phdata)
                                <option value="{{ $phdata['phone'] }}" data-length="{{ json_encode($phdata['phoneLength']) }}" {{ (old('parent_phone_code', auth('student')->user()->parent_phone_code) == $phdata['phone'] and strlen(auth('student')->user()->parent_phone_number) == json_encode($phdata['phoneLength']) ) ? 'selected' : '' }}>{{ $phdata['label'].' - '.$phdata['phone'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-7">
