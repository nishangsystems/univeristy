...
@section('section')
    ...
    <div class="col-sm-12">
        <div class="d-flex flex-wrap justify-content-between alert alert-info text-center text-uppercase my-3"><span>{{__('text.total_paid').' : '.number_format($student->total_paid( $year)).' '.__('text.currency_cfa')}}</span><span>{{__('text.total_fee_owed').' : '.number_format($student->total_debts($year)).' '.__('text.currency_cfa')}}</span></div>
        ...
        <div class="content-panel">
            @forelse($student->payments()->where(['payment_year_id'=>(request('year') ?? \App\Helpers\Helpers::instance()->getYear())])->get() as $item)
                @if ($item->amount != 0 && $item->amount != null)
                    <div class="card border bg-light py-3 px-5 d-flex justify-content-between my-4 align-items-end">
                        ...
                        <div class="d-none">
                            <div id="printHERE{{$item->id}}" class="eachrec watermark-bg">
                ...
                @if($item->debt != 0 && $item->debt != null)
                    <div class="card border bg-light py-3 px-5 d-flex justify-content-between my-4 align-items-end">
                        ...
                        <button class="btn btn-sm btn-primary" onclick="printDiv('_printHERE{{$item->id}}')">{{__('text.word_print')}}</button>
                        <!-- create a hidden div for printable markup and print with js on request -->
                        <div class="d-none">
                            <div id="_printHERE{{$item->id}}" class="eachrec watermark-bg">
