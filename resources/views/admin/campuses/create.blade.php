    ...
    <div class="col-sm-12">
        <div class="content-panel">
            <div class="py-4 my-3 form-panel">
                <form class="form-horizontal" role="form" action="{{route('admin.campuses.store')}}" method="post" >
                    @csrf
                    ...
                    <div class="form-group @error('telephone') has-error @enderror my-2">
                        ...
                        <div class="col-lg-10">
                            <input class=" form-control" name="telephone" value="{{old('telephone')}}" type="tel"  />
                            @error('telephone')
                            <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <tr>
                            ...
                            <td>
                               @if($cps->students()->count() > 0)
                                    <a href="{{route('admin.campuses.edit', $cps->id)}}" class="btn btn-sm btn-warning text-capitalize">{{__('text.word_edit')}}</a>
                               @else
