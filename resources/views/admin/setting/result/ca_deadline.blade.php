    ...
    <form method="post" class="card mx-5 my-5">
        ...
        <div class="card-body">
            ...
            <div class="row my-2">
                ...
                <div class="col-sm-9">
                    <input class="form-control" name="ca_upload_latest_date" type="date" required value="{{ old('ca_upload_latest_date') }}">
                </div>
            </div>
    ...
    <table class="table">
        ...
        @foreach($semesters as $key => $semester)
            <tr>
                <td>{{ $k++ }}</td>
                <td>{{ $semester->name??'' }}</td>
                <td>{{ $semester->ca_upload_latest_date == null ? null :$semester->ca_upload_latest_date->format('d-m-Y') }}</td>
            </tr>
        @endforeach
    </table>
</div>
