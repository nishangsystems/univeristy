...
<div class="mx-3">
    <div class="form-panel">
        <div class="row">
            <div class="col-lg-5">
                @if($student->registration_bal() > 0)
                    <form class="form-horizontal" role="form" method="POST">
                        <h5 class="mt-5 font-weight-bold text-capitalize">{{__('text.enter_fee_details')}}</h5>
                        @csrf
                        <div class="form-group">
                            <label for="cname" class="control-label col-lg-2 text-capitalize">{{__('text.total_fee')}}: </label>
                            <div class="col-lg-10">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-10">
                                <input for="cname" class="form-control" name="balance" value="{{number_format($balance)}} CFA" disabled>
                            </div>
                        </div>
                        <div class="form-group @error('item') has-error @enderror">
                            ...
                            <div class="col-lg-10">
                                <select class="form-control" name="item">
                                    ...
                                    @foreach($student->class(\App\Helpers\Helpers::instance()->getYear())->payment_items()->where('name', 'REGISTRATION')->where(['year_id'=>$c_year])->get() ?? [] as $item)
                                        <option value="{{$item->id}}" data-amount="{{ $item->amount }}" selected>{{$item->name." - ".$item->amount}} FCFA</option>
                                    @endforeach
                                </select>
                                @error('item')
                        ...
                        <div class="form-group @error('amount') has-error @enderror">
                            ...
                            <div class="col-lg-10">
                                <input class=" form-control" name="amount" value="{{ $balance }}" required readonly />
                                @error('amount')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-10">
                                <input class=" form-control" name="date" value="{{old('amount', now())}}" type="date" required />
                            </div>
                        </div>
                        <div class="form-group">
                ...
                @else
                    <div class="alert alert-success text-center text-capitalize">{{__('text.phrase_registrattion_fee_complete')}}</div>
                @endif
            </div>
