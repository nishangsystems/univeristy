...
@else
    @php
        ...
        $courses = \App\Models\ProgramLevel::find(request('class_id'))->class_subjects ?? [];
                ...
                <tbody>
                    @foreach($courses as $course)
                        <tr class="border-top border-bottom border-secondary">
                            ...
                            <td class="border-left border-right border-secondary">{{$course->subject->student_subjects()->count()}}</td>
                            <td class="border-left border-right border-secondary">{{$course->results()->distinct()->count()}}</td>
                            <td class="border-left border-right border-secondary">{{$course->results()->where(function($q){$q->first()->passed() == true;})->count()}}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
