                    ...
                    data.forEach(element => {
                        tbody_content += `<tr class="border-top border-bottom border-secondary">
                            ...
                            <td class="border-left boder-right border-secondary">
                                <a class="btn btn-sm btn-primary" target="_new" onclick="$('#print_trans_${k}').submit()">{{__('text.word_print')}}</a>
                                <form method="get" class="hidden" action="{{route('admin.res_and_trans.print_transcript')}}" id="print_trans_${k}">
                                    <input type="hidden" name="student_matric" value="${element.matric}">
                                </form>
                            </td>
                        </tr>`;
                    });
