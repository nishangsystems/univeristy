    ...
    <form action="{{route('admin.students.promote')}}" method="post" class="w-100">
        ...
        <div class="w-100 py-2">
            <!-- <div class="d-flex justify-content-center align-moddle">
                ...
                <span class="text-dark fw-bold fs-3" id="student-total">58</span>
            </div> -->
            <div class="w-100">
                
                <table class="w-100 table-striped">
                    <thead class="w-100 bg-secondary text-light">
                        <th class="border-left border-right"><input type="checkbox" id="" onchange="updateSelection(event)"><span>all</span></th>
                    ...
                    <tbody>
                        ...
                        <tr class="fw-bolder fs-3 text-dark border-bottom">
                            <td class="border-left border-right"><input type="checkbox" class="checker" name="students[]" value="{{$student->id}}" id=""></td>
...
<script>
    function updateSelection(event){
        if (event.target.checked) {
            document.querySelectorAll(".checker").forEach((value, key, parent)=>{value.setAttribute('checked', true);});
        }
        else{
            document.querySelectorAll(".checker").forEach((value, key, parent)=>{value.setAttribute('checked', false);});
        }
    }
</script>
@endsection