        ...
        @if($semester_id != null)
            <div class="card my-5 shadow border-left-0 border-right-0 border-top border-bottom border-dark">
                ...
                <div class="card-body py-5 my-2">
                    <form class="row my-2" method="post" action="{{ route('user.results.alter_results.change.ca', ['semester_id'=>$semester_id, 'student_id'=>$student_id, 'year_id'=>$year_id]) }}">
                        @csrf
                        <div class="col-lg-6">
...
@section('script')
    <script>
        ...
        let presubmit_submit = function(element){
            ...
            let url = "{{ route('user.results.alter_results.change', ['year_id'=>'_YR_', 'semester_id'=>'_SMST_', 'student_id'=>'_SID_']) }}".replace('_YR_', year).replace('_SMST_', semester).replace('_SID_', `${sid}`);
            // alert(url);
            window.location = url;
        }
