                ...
                <tbody>
                    ...
                    @if(\request('class'))
                        ...
                        <tr>
                            ...
                            <td style="float: right; text-transform: capitalize">
                                <a class="btn btn-xs btn-success mt-1 text-capitalize" href="{{route('course.notification.index', [$subject->id])}}">{{__('text.word_notifications')}}</a>
                                <a class="btn btn-xs btn-primary mt-1 text-capitalize" href="{{route('user.subject.students', [$subject->class, $subject->id])}}?campus_id={{$subject->campus_id}}">{{__('text.word_students')}}</a>
                                <a class="btn btn-xs btn-success mt-1 text-capitalize" href="{{route('user.subject.show', [$subject->class, $subject->id])}}">{{__('text.upload_material')}}</a>
                                <a class="btn btn-xs btn-primary mt-1" href="{{route('user.class_course.ca.result', [$subject->class, $subject->id])}}"><i class="fa fa-tick"> {{__('text.word_results')}}</i></a>
                            </td>
                        </tr>
                        @endforeach
                    @else
